//! 这个模块提供了访问当前正在运行的上下文的方法。
//! 

use core::ptr::{addr_of, NonNull};
use alloc::boxed::Box;
use crate::{context::TaskContextRef, TaskContext};


// pub fn current_task_ctx() -> TaskContextRef {
//     let ptr = current_task_ptr::<TaskContext>();
//     TaskContextRef(unsafe { NonNull::new_unchecked(ptr as *mut _) })
// }

pub fn set_current_task_ctx(task_ctx_ref: TaskContextRef) {
    // log::error!("{:#X}", unsafe { addr_of!(__PERCPU_CURRENT_TASK_PTR) as *const _ as usize });    
    let ptr = task_ctx_ref.0.as_ptr() as *const TaskContext;
    log::info!("set_current_task_ctx {:#X?}", ptr as usize);
    unsafe { set_current_task_ptr(ptr) }
    log::info!("{:#X?}", CURRENT_TASK_PTR.read_current());
}
...
pub fn current_task_ptr<T>() -> *const T {
    ...
    unsafe {
        ...
        log::error!("current_task_ptr {:#X?}", ans);
